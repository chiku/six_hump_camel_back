  ...
  describe "when rand() returns 0.5" do
    ...
    describe "#==" do
      it "doesn't equal another vector with different members" do
        (vector == SixHumpCamelBack::Vector.new(2, 0)).must_equal false
        (vector != SixHumpCamelBack::Vector.new(2, 0)).must_equal true
      end

      it "equals a vector with same members" do
        (vector == SixHumpCamelBack::Vector.new(1, 0)).must_equal true
        (vector != SixHumpCamelBack::Vector.new(1, 0)).must_equal false
      end
    ...
    describe "#hash" do
      it "equals another vector with same members" do
        vector.hash.must_equal SixHumpCamelBack::Vector.new(1, 0).hash
      end
    end

    describe "#-" do
      it "is the difference of individual elements" do
        (vector - another_vector).must_equal SixHumpCamelBack::Vector.new(1, -1)
      end
    end
    ...
    describe "#scale_by" do
      it "is the product of the members by the scalar number" do
        vector.scale_by(2).must_equal SixHumpCamelBack::Vector.new(2, 0)
      end
    end
    ...
    describe "#crossover_with" do
      ...
      it "is composed of elements from itself and other vector selected by randomization" do
        vector.crossover_with(another_vector, randomization: randomization, factor: 0.5).must_equal SixHumpCamelBack::Vector.new(0, 0)
      end
