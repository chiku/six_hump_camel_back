...
describe "Vector" do
  describe "members" do
    it "lie between constraints" do
      10.times do
        ...
        vector = Vector.new(5, constraints, [0, 2.2, 3, 5, -1], [2, 4.5, 5, 6, 0], lambda {|i, j, k, l, m| i + j})
    ...
    describe "when constraints are equal" do
      it "generate the same value" do
        10.times do
          ...
          vector = Vector.new(2, constraints, [0, 2.2], [0, 2.2], lambda {|i, j| i + j})
    ...
    it "can be set" do
      ...
      vector = Vector.new(2, constraints, [0, 2.2], [1, 2.2], lambda {|i, j| i + j})
      vector[0] = 100
  ...
  it "has a fitness" do
    ...
    vector = Vector.new(2, constraints, [1, 3.2], [1, 3.2], lambda {|i, j| i + j})
  ...
  describe "with three member" do
    it "has a fitness" do
      ...
      vector = Vector.new(3, constraints, [33, 3.2, 10], [33, 3.2, 10], lambda {|i, j, k| i - j * k})
  ...
  it "doesn't equal another vector with different degree" do
    ...
    vector = Vector.new(2, constraints, [33, 3.2], [33, 3.2], lambda {|i, j| i + j})
    other_constraints = [Constraint.new(min: 33, max: 33), Constraint.new(min: 3.2, max: 3.2), Constraint.new(min: 10, max: 10)]
    other_vector = Vector.new(3, other_constraints, [33, 3.2, 10], [33, 3.2, 10], lambda {|i, j, k| i + j})
  ...
  it "equals a vector with same members" do
    ...
    vector = Vector.new(3, constraints, [33, 3.2, 10], [33, 3.2, 10], lambda {|i, j, k| i - j * k})
    other_constraints = [Constraint.new(min: 33, max: 33), Constraint.new(min: 3.2, max: 3.2), Constraint.new(min: 10, max: 10)]
    other_vector = Vector.new(3, constraints, [33, 3.2, 10], [33, 3.2, 10], lambda {|i, j, k| i + j * k})
  ...
  it "doesn't equal a vector with one different member" do
    ...
    other_vector = Vector.new(3, other_constraints, [33, 3.2, 0], [33, 3.2, 0], lambda {|i, j, k| i - j * k})
    assert(vector != other_vector)
  end
end
