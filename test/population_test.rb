...
describe "Population" do
  describe "the first vector" do
    it "is accessible" do
      ...
      population = Population.new(1, [1, 2], [1, 2], lambda {|i, j| i + j})
  ...
  describe "with one vector" do
    it "has a fitness" do
      population = Population.new(1, [1.1, 2.2], [1.1, 2.2], lambda {|i, j| i + j})
      assert_in_delta(3.3, population[0].fitness, 0.00001)
    end
  end

  it "has a difference vector" do
    population = Population.new(3, [1.0], [1.0], lambda {|i| i + 2})
  ...
  describe "with fitness criteria x squared plus one" do
    it "has a minima at zero" do
      population = Population.new(100, [-100], [100], lambda {|x|  x*x+1  })
  ...
  describe "for camel hump back problem" do
    it "has a solution" do
      ...
      population = Population.new(100, [-3.0, -2.0], [3.0, 2.0], lambda {|x, y|  ( 4.0 - 2.1*x*x +  x*x*x*x/3.0) * x*x + x*y + ( -4.0  +  4.0*y*y) * y*y })
